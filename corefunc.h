...
int START_DARI = 10;
//acc password
string Password_acc = "!Password!2a3";
//acc username
string Growid_acc = "Mengonntol";
	    	...
	    	if (extended) {
	    		...
	    		if (func == "OnRequestWorldSelectMenu") {
	    			...
	    			if (ctx.find("add_label_with_icon|big|`wGet a GrowID``|left|206|") != std::string::npos) {
	    				SendPacket(2, "action|dialog_return\ndialog_name|growid_apply\nlogon|"+ Growid_acc + to_string(START_DARI) + "\npassword|"+ Password_acc +"\npassword_verify|"+ Password_acc +"\nemail|Zaqxkun88" + to_string(rand() % 50000) + "@gmail.com", peer);
	    		START_DARI = START_DARI + 1;
              	}
	    				...
	    				string passwr = "Password";
	    				cout << "Success Make Account Growid: " << growid << ", Password: " << Password_acc << endl;
	    			...
	    			if (varlist.size() > 2) { //input text
		    			...
		    			if (str.find("!rate") != std::string::npos) {
		    				SendPacket(2, "action|input\n|text|/rate 5.99", peer);
		    				SendPacket(2, "action|input\n|text|Im Rate 5.99", peer);
		    			}
		    			if (str.find("!bot") != std::string::npos) {
		    				PlayerMoving data;
							data.packetType = 0;
							data.characterState = 3104;
							data.x = localX;
							data.y = localY;
							data.netID = localid;
							data.punchX = (localX / 32) + 1;
							data.punchY = localY / 32;
							data.XSpeed = 0;data.YSpeed = 0;
							data.plantingTree = 822;
							SendPacketRaw(4, packPlayerMoving(&data), 56, 0, peer, ENET_PACKET_FLAG_RELIABLE);
							SendPacket(2, "action|input\n|text|x: " + to_string(localX / 32) + " y: " + to_string(localY / 32), peer);
		    			}
		    			if (str.find("`0`1`4`7`w!goto ") != std::string::npos) {
		    				string wname = explode("!goto ", str)[1];
		    				SendPacket(2, "action|input\n|text|Warping to " + wname, peer);
		    				SendPacket(3, "action|join_request\nname|" + wname, peer);
		    			}
		    			if (str.find("!stopfollow") != std::string::npos) {
		    				if (netid == follownetid) {
		    					follownetid = -1;
		    				}
		    			}
		    			if (str.find("`1`w!follow") != std::string::npos) {
		    				follownetid = netid;
		    			}
		    			else if (netid == follownetid) {
		    				if (str.find("OID:_") != std::string::npos) {
		    						auto t = explode("OID:_", str);
		    						string t2 = t[t.size()-1];
		    						SendPacket(2, "action|input\n|text|" + t2, peer);
		    				}
		    			}
	    			}
	    			...
	    			if (content.find("[MSG]_") !=std::string::npos) {
	    				if (content.find("!home") !=std::string::npos) {	    					
	    					cout << uname << " Warping to " << currentWorld << endl;
	    				}
	    			}
	    			//if (content.find("[SB]_") == std::string::npos) cout << content << endl;
	    		}
			}
		}
	}
